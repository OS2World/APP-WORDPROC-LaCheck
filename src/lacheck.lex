 ...
 *	
 * ** NOTE: if anyone makes this compile on VMS I'd like to know ***
 * 
 * Copyright (C) 1991, 1992 Kresten Krab Thorup.
 * Copyright (C) 1993 Per Abrahamsen.
 *
 * Please send bugs, suggestions, and queries to auc-tex_mgr@iesd.auc.dk.
 * 
 * $Revision: 1.8 $
...
<ICOR,INITIAL,GETICOR>{ws}{non_abbrev}/{non_par_ws}{u_letter}   { 
 {
    printf("\"%s\", line %d: missing `\\@' before punctuation mark in \"%s\"\n", 
	   file_name, line_count, ++yytext); 
    ++warn_count ; 
	linecount();
    BEGIN(INITIAL);
...
<ICOR,INITIAL,GETICOR>{ws}"\\"(ref|cite)/[^A-Za-z]  {
    linecount();
